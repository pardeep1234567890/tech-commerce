...
const CartModal = () => {
    ...
    return (
        <>
            ...
            {/* Cart Modal */}
            <div
                className={`fixed right-0 top-0 z-50 h-full w-full max-w-md transform bg-white shadow-xl transition-transform ${isCartOpen ? 'translate-x-0' : 'translate-x-full'
                    }`}
            >
                <div className="flex h-full flex-col">
                    ...
                    <div className="flex-grow overflow-y-auto p-4">
                        {cartItems.length === 0 ? (
                            <p className="mt-4 text-center text-gray-600">Your cart is empty.</p>
                        ) : (
                                        ...
                                        <div className="ml-4 flex flex-1 flex-col">
                                            ...
                                            <div className="flex flex-1 items-end justify-between text-sm">
                                                <div className="flex items-center border">
                                                    <button onClick={() => updateQuantity(item._id, item.qty - 1)} className="p-1.5 text-gray-600 hover:bg-gray-100"><Minus size={16} /></button>
                                                    <span className="w-8 text-center">{item.qty}</span>
                                                    <button onClick={() => updateQuantity(item._id, item.qty + 1)} className="p-1.5 text-gray-600 hover:bg-gray-100"><Plus size={16} /></button>
                                                </div>
                                                <button
                                                    type="button"
                    ...
                    <div className="border-t p-4">
                        ...
                        <Link
                            ...
                            className="mt-6 flex w-full items-center justify-center rounded-md bg-black px-6 py-3 text-base font-medium text-white transition-opacity hover:opacity-80"
                        >
                            Checkout
                        </Link>
                    </div>
